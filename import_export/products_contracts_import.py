...
def import_products_from_contracts_csv(csv_path: str, progress_callback=None) -> Dict[str, int]:
    """
    ...
    for row in rows:
        ...
        if progress_callback:
            progress_callback(current_row, total_rows, f"Обработка строки {current_row}/{total_rows}")
        ...
        if any(keyword in full_text for keyword in ["ООО", "АО", "ОАО", "ЗАО", "РПТП", "КСК", "УРАЛТРАНСМАШ", "МК ВИТЯЗЬ", "78 ЦЕНТРАЛЬНАЯ ИНЖЕНЕРНАЯ БАЗА", "МПЗ"]):
        ...
        for product_key, product_data in products_data.items():
            try:
                ...
                if product_data["contract_name"]:
                    contract = q.get_contract_by_name(conn, product_data["contract_name"])
                    if contract:
                ...
                if not contract_id:
                    ...
                    if not no_contract:
                        contract_id = q.insert_contract(
                            conn, "Без контракта", None, None, "Автоматически создан для изделий без контракта",
                            name="Без контракта", contract_type="Системный", executor="Система"
                        )
                    else:
...
def _save_current_group(current_group, products_data, contracts_data):
    ...
    product_key = f"{current_group['product']['name']}_{current_group['product']['number']}"
    ...
    if product_key not in products_data:
        products_data[product_key] = {
            ...
            "contract_name": current_group["contract"]["name"] if current_group["contract"] else None,
            "contract_number": current_group["contract"]["number"] if current_group["contract"] else None
        }
    
    # Сохраняем контракт если есть
    if current_group["contract"]:
        contract_key = f"{current_group['contract']['name']}_{current_group['contract']['number']}"
...
def generate_products_contracts_template(output_path: str) -> str:
    ...
    template_data = [
        ...
        ["Двигатель В-55 У № 307Д5012", "УРАЛТРАНСМАШ АО", "2224187314431432245222903 Д/С №2 ( УТМ 2024)", "29.05.2023"],
        ["Двигатель 4ч 8,5/11 № 7706313", "ОВК ООО", "Договор № 2325187913551442245231239/23 (ОВК 2024)", "14.08.2023"]
    ]
