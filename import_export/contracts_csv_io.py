...
def import_contracts_from_csv(conn, file_path: str | Path) -> tuple[int, int]:
        ...
        with open(file_path, "r", encoding="utf-8-sig") as file:
            ...
            for row_num, row in enumerate(
                ...
                try:
                    ...
                    start_date_str = clean_text(
                        row.get("Дата заключения контракта", "")
                    )
                    end_date_str = clean_text(
                        row.get("Плановая дата исполнения контракта", "")
                    )
    ...
    logger.info(
        f"Импорт завершен. Импортировано: {imported_count}, обновлено: {updated_count}"
    )
    ...
    with open(file_path, "w", newline="", encoding="utf-8-sig") as file:
        ...
        examples = [
            [
                ...
                "ОВК 239/13/ГОЗ-23",
                "Гособоронзаказ (ГОЗ)",
                "МАЛМЫЖСКИЙ РЕМЗАВОД АО",
                "2325187913551442245231239",
                "2325187913551442245231239/739-1/55/13983/5125/2324/13/ГОЗ-23",
                '40706810003000140827, Приволжский ф-л ПАО "Промсвязьбанк"',
                "24.07.2023",
                "31.12.2023",
                "",
            ],
            [
                ...
                "МАЛМЫЖСКИЙ РЕМЗАВОД АО",
                "2325187913551442245231239",
                "2325187913551442245231239/27/ГОЗ-24",
                '40706810003000198992, Приволжский ф-л ПАО "Промсвязьбанк"',
                "14.06.2024",
                "31.12.2025",
                "",
            ],
