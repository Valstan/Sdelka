...
def _parse_orders_sheet(df: pd.DataFrame, sheet_name: str | None = None) -> list[dict]:
    ...
    for i in range(max_rows_scan):
        ...
        for ii in range(i, min(i + 3, max_rows_scan)):
            ...
            for j, s in enumerate(vals):
                ...
                if (
                    s_low.startswith("таб№")
                    or s_low == "таб№"
                    or s_low.startswith("таб#")
                ):
    ...
    header_text = "\n".join(
        [
            ";".join([_norm_str(v) for v in df.iloc[i, :].tolist()[:8]])
    ...
    if header_idx is not None:
        ...
        for _, r in data.iterrows():
            ...
            if not first_cell and any(
                "изделие №" in _norm_str(c).lower() for c in cells[:3]
            ):
                ...
                low_first = next(
                    (
                        _norm_str(c).lower()
                        for c in cells
                        if "изделие №" in _norm_str(c).lower()
                    ),
                    low_first,
                )
                first_cell = next(
                    (c for c in cells if "изделие №" in _norm_str(c).lower()),
                    first_cell,
                )
            ...
            if low_first.startswith("изделие №"):
                ...
                for part in raw_parts:
                    ...
                    s = re.sub(
                        r"(?i)-(повторно|повтор|правая|левая|рекламационный|рекламация|правый|левый)\b.*$",
                        "",
                        s,
                    ).strip()
            ...
            if m:
                ...
                yy = (
                    int(year)
                    if yr is None
                    else int(("20" + yr) if len(yr) == 2 else yr)
                )
                from datetime import datetime as _dt

                try:
                    date_str = _dt(int(yy), int(mth), int(d)).strftime(
                        CONFIG.date_format
                    )
                except Exception:
                ...
                current_group = {
                    "date": date_str,
                    "products": list(current_products),
                    "items": [],
                    "workers": workers,
                }
                continue
...
def import_xlsx_full(
    file_path: str | Path,
    progress_cb: callable | None = None,
    *,
    include_jobtypes: bool = True,
    ...
    with get_connection() as conn:
        ...
        for sheet in xls.sheet_names:
            ...
            is_jobtypes = (
                any("наименование" in c for c in lower_cols)
                and any("ед" in c for c in lower_cols)
                and any(("цена" in c or "расценка" in c) for c in lower_cols)
            )
            ...
            has_price_or_sum = any(
                ("цена" in c or "расценка" in c or "сумма" in c) for c in lower_cols
            )
            ...
            try:
                ...
                for i in range(scan_rows):
                    row_vals = [
                        _norm_str(x).lower()
                        for x in (df.iloc[i, :scan_cols].tolist() if scan_cols else [])
                    ]
            ...
            is_orders = (
                has_date and has_name_col and has_unit and has_price_or_sum
            ) or has_izdelie
            ...
            if include_jobtypes and (is_jobtypes and not is_orders):
                ...
                for j in jobs:
                    if not j["name"]:
                        continue
                    q.upsert_job_type(
                        conn, j["name"], j["unit"] or "шт.", float(j["price"])
                    )
                    jt_count += 1
                continue
            ...
            if include_orders and is_orders:
                ...
                for g in groups:
                    ...
                    date_str = g.get("date") or datetime.now().strftime(
                        CONFIG.date_format
                    )
                    if not items:
                    ...
                    for it in items:
                        ...
                        jt_row = (
                            q.get_job_type_by_name(conn, it["job_name"])
                            or conn.execute(
                    ...
                    code = (
                        _norm_str(mcode.group(2))
                        if mcode
                        else f"ИМПОРТ_{datetime.now().year}"
                    )
                    try:
                    ...
                    c_row = conn.execute(
                        "SELECT id FROM contracts WHERE code=?", (code,)
                    ).fetchone()
                    contract_id = (
                        int(c_row[0])
                        if c_row
                        else q.get_or_create_default_contract(conn)
                    )
                    ...
                    for w in workers or []:
                        ...
                        if roww:
                            worker_inputs_template.append(
                                {
                                    "worker_id": int(roww["id"]),
                                    "worker_name": nm,
                                    "amount": None,
                                }
                            )
                    ...
                    if not worker_inputs_template:
                        ...
                        worker_inputs_template = [
                            {"worker_id": int(wid), "worker_name": nm, "amount": None}
                        ]
                    ...
                    for idx in range(n):
                        ...
                        if prod_no_clean:
                            ...
                            prod_cid = contract_id or q.get_or_create_default_contract(
                                conn
                            )
                            ...
                            if existing:
                                ...
                                try:
                                    ...
                                    q.upsert_product(
                                        conn, prod_name, prod_no_clean, prod_cid
                                    )
                                    products_count += 1
                                except Exception as exc:
                                    logging.getLogger(__name__).exception(
                                ...
                                rowp = conn.execute(
                                    "SELECT id FROM products WHERE product_no_norm = ?",
                                    (_norm_str(prod_no_clean).casefold(),),
                                ).fetchone()
                                if rowp:
                                    try:
                        ...
                        wo_items_inputs = [
                            WorkOrderItemInput(job_type_id=jt_id, quantity=qty)
                        ]
                        wo_workers = [
                            WorkOrderWorkerInput(
                            ...
                            for w in worker_inputs_template
                        ]
                        data = WorkOrderInput(
                            date=date_str,
                            product_id=product_id,
                            contract_id=contract_id,
                            items=wo_items_inputs,
                            workers=wo_workers,
                        )
                        try:
                            ...
                            logger.exception(
                                "Не удалось создать наряд с листа %s: %s", sheet, e
                            )
                continue
    ...
    for row in df.itertuples(index=False):
        ...
        count += (
            q.upsert_worker(
                ...
                str(row.full_name),
                _opt_str(row.dept),
                _opt_str(row.position),
                str(row.personnel_no),
                status=status_val,
            )
            or 0
        )
...
def import_job_types_from_excel(conn: sqlite3.Connection, file_path: str | Path) -> int:
    ...
    for row in df.itertuples(index=False):
        count += (
            q.upsert_job_type(conn, str(row.name), str(row.unit), float(row.price)) or 0
        )
            ...
            if code_str:
                ...
                if r:
                    try:
                        ...
                        contract_id = None
        count += (
            q.upsert_product(conn, str(row.name), str(row.product_no), contract_id) or 0
        )
    ...
    for row in df.itertuples(index=False):
        count += (
            q.upsert_contract(
                conn,
                str(row.code),
                _opt_str(row.start_date),
                _opt_str(row.end_date),
                _opt_str(row.description),
            )
            or 0
        )
...
def export_table_to_excel(
    conn: sqlite3.Connection, table: str, file_path: str | Path
) -> Path:
    ...
    if table == "workers":
        ...
        df.sort_values(
            by=["_status_key", "_dept_key", "_pos_is_head", "_pos_name", "full_name"],
            inplace=True,
            kind="mergesort",
        )
        df.drop(
...
def export_all_tables_to_excel(
    conn: sqlite3.Connection, dir_path: str | Path
) -> list[Path]:
...
def analyze_orders_workbook(file_path: str | Path) -> list[dict]:
    ...
    for sheet in xls.sheet_names:
        ...
        is_orders = any(
            "фио" in _norm_str(v).lower() for v in df.head(5).iloc[:, 0].tolist()
        ) or any("дата" in c for c in lower_cols)
        if is_orders:
...
def generate_contracts_template(file_path: str | Path) -> Path:
    df = pd.DataFrame(
        {"Шифр контракта": [], "Дата начала": [], "Дата окончания": [], "Описание": []}
    )
    file_path = Path(file_path)
    df.to_excel(file_path, index=False)
    return file_path
