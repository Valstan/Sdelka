## 3.11.3 от 21 сентября 2025 года

### КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ
- **ИСПРАВЛЕНО ДУБЛИРОВАНИЕ ДАННЫХ**: Кардинально изменена логика синхронизации при запуске программы
- **Синхронизация при запуске**: Теперь НЕ объединяет данные если локальная БД уже существует (предотвращает дублирование)
- **Проверка дублей нарядов**: Добавлена проверка существования нарядов по дате и контракту при объединении БД
- **Принудительная синхронизация**: Красная кнопка "Обновить данные" теперь делает РЕАЛЬНОЕ объединение данных
- **Сохранность справочников**: Улучшена логика upsert для предотвращения потери данных

### Исправлено
- Устранено постоянное дублирование нарядов при каждом запуске программы
- Исправлена потеря данных справочников при синхронизации
- Улучшена стабильность работы виджетов CustomTkinter
- Добавлены проверки существования виджетов для предотвращения TclError

### Изменено
- Синхронизация при запуске стала условной - только если локальной БД нет
- Объединение данных происходит только при принудительной синхронизации (красная кнопка)
- Улучшено логирование процессов объединения БД

## 3.11.2 от 21 сентября 2025 года

### Исправлено
- Убран проблемный диалог синхронизации при запуске программы
- Синхронизация при запуске теперь работает в фоне без блокировки интерфейса
- Создан универсальный диалог синхронизации для принудительных операций (красная кнопка, закрытие программы)
- Упрощена логика версионирования синхронизации - убраны файлы версий, вызывавшие 404 ошибки
- Исправлены ошибки зацикливания синхронизации
- Улучшена стабильность работы виджетов и жизненного цикла объектов
- Исправлены множественные TclError при работе с canvas элементами

### Изменено
- Синхронизация при запуске стала фоновой - программа открывается сразу
- Статус синхронизации отображается в главном окне программы
- Упрощена архитектура синхронизации - убраны сложные проверки версий

## 3.11.1 от 21 сентября 2025 года

### Исправлено
- Добавлен токен Яндекс.Диска по умолчанию для автоматической синхронизации
- Исправлена логика синхронизации при запуске: теперь всегда объединяет локальную и удаленную БД (не замещает)
- Добавлена синхронизация при выключении программы с диалогом прогресса
- Создан диалог прогресса синхронизации с визуальной обратной связью
- Улучшена обработка случаев отсутствия токена или БД

### Добавлено
- Модуль `gui/sync_progress_dialog.py` с всплывающим окном прогресса
- Класс `SyncProgressManager` для управления диалогами синхронизации
- Автоматическое закрытие диалога прогресса через 3 секунды при успехе
- Возможность отмены синхронизации через диалог
