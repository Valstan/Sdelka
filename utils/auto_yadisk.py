...
def _next_trigger_time(hours: list[int]) -> datetime:
    ...
    # next day first hour
    first = today_hours[0]
    next_day = (now + timedelta(days=1)).replace(hour=first, minute=0, second=0, microsecond=0)
    return next_day
...
def _upload_job() -> None:
    try:
        ...
        if not token:
            logger.warning("Auto Yadisk: пропуск — токен не задан ни в настройках, ни в CONFIG")
            return
        src_db = Path(get_current_db_path())
        backup_path = backup_sqlite_db(src_db)
        if not backup_path:
            logger.error("Auto Yadisk: не удалось создать локальный бэкап — выгрузка пропущена")
            return
        ...
        logger.exception("Auto Yadisk: ошибка выгрузки")
...
def _scheduler_loop(hours: list[int]) -> None:
    ...
    while True:
        try:
            ...
            logger.exception("Auto Yadisk: ошибка в цикле планировщика")
...
def start_yadisk_upload_scheduler() -> None:
    ...
    th = threading.Thread(target=_scheduler_loop, args=(hours,), daemon=True, name="YadiskScheduler")
    th.start()
