...
def detect_sheet(df: pd.DataFrame) -> tuple[DocKind, int, dict[str, Any] | None]:
    ...
    # Scoring approach
    scores: dict[DocKind, int] = {"orders": 0, "job_types": 0, "products": 0, "contracts": 0, "workers": 0, "unknown": 0}
    hints: dict[str, Any] = {}
    ...
    if has_any(["дата"]):
        scores["orders"] += 1
    if has_any(["вид", "работ"]) and has_any(["ед"]) and has_any(["цена", "расценка"]) and has_any(["сумма"]):
        scores["orders"] += 3

    # Contracts
    if has_any(["номер контракта", "контракт", "договор", "шифр"]):
        scores["contracts"] += 2
    if has_any(["игк", "исполнител", "вид контракт", "начал", "заключ", "оконч", "действует до"]):
        scores["contracts"] += 2
    ...
    if has_any(price_keys):
        scores["job_types"] += 2
    if (("вид" in head_text or "наимен" in head_text) and any(k in head_text for k in unit_keys) and any(k in head_text for k in price_keys)):
        scores["job_types"] += 2

    # Products
    if has_any(["изделие", "зав", "серийн", "изд."]):
        scores["products"] += 2
    if has_any(["№", "номер"]) and has_any(["контракт", "договор", "шифр", "номер контракта"]):
        scores["products"] += 2
    ...
    def _guess_job_types_hints() -> dict[str, Any]:
        ...
        unit_col = next((c for c in df.columns if any(k in str(c).lower() for k in unit_keys)), None)
        price_col = next((c for c in df.columns if any(k in str(c).lower() for k in price_keys)), None)
    ...
    def _guess_workers_hints() -> dict[str, Any]:
        ...
        return {"fio_col": fio_col or "+Ф/И/О", "fam_col": fam_col, "im_col": im_col, "otch_col": otch_col, "personnel_no_col": tab_col, "dept_col": dept_col}
