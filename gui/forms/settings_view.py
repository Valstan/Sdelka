...
class SettingsView(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ...
        self._btn_export_db = ctk.CTkButton(
            btns, text="Сохранить копию базы...", command=self._export_db
        )
        self._btn_export_db.pack(side="left", padx=6)
        self._btn_merge_db = ctk.CTkButton(
            btns, text="Слить с другой базой...", command=self._merge_db
        )
        self._btn_merge_db.pack(side="left", padx=6)
        # Импорт данных перенесён сюда
        self._btn_import_unified = ctk.CTkButton(
            btns, text="Импорт данных", command=self._import_unified
        )
        ...
        ctk.CTkLabel(self._yadisk_section, text="Бэкап на Яндекс-диск").pack(
        ...
        default_token = (
            self._prefs.yandex_oauth_token or CONFIG.yandex_default_oauth_token or ""
        )
        self._yd_token_var = ctk.StringVar(value=default_token)
        self._yd_token_entry = ctk.CTkEntry(
            row_token, textvariable=self._yd_token_var, show="*"
        )
        self._yd_token_entry.pack(side="left", fill="x", expand=True, padx=6)
        ctk.CTkButton(
            row_token, text="Сохранить", command=self._save_yadisk_settings
        ).pack(side="left", padx=6)
        ctk.CTkButton(
            row_token, text="Вставить из буфера", command=self._paste_yd_token
        ).pack(side="left", padx=6)
        ctk.CTkButton(row_token, text="Очистить", command=self._clear_yd_token).pack(
            side="left", padx=6
        )
        ...
        self._yd_dir_var = ctk.StringVar(
            value=self._prefs.yandex_remote_dir
        ...
        self._btn_upload_yadisk = ctk.CTkButton(
            row_dir,
            text="Выгрузить бэкап на Яндекс.Диск",
            command=self._upload_latest_backup_to_yadisk,
        )
        self._btn_upload_yadisk.pack(side="left", padx=6)
        ctk.CTkButton(
            row_dir, text="Проверить токен", command=self._check_yadisk_token
        ).pack(side="left", padx=6)

        # Приватный путь к файлу на Диске
        row_priv = ctk.CTkFrame(self._yadisk_section)
        row_priv.pack(fill="x", pady=(2, 6))
        ctk.CTkLabel(row_priv, text="Приватный путь к файлу (.db)").pack(
            side="left", padx=6
        )
        self._yd_private_var = ctk.StringVar(
            value=self._prefs.yandex_private_file_path
            or "/SdelkaBackups/sdelka_base.db"
        )
        self._yd_private_entry = ctk.CTkEntry(
            row_priv, textvariable=self._yd_private_var
        )
        self._yd_private_entry.pack(side="left", fill="x", expand=True, padx=6)
        ctk.CTkButton(
            row_priv, text="Сохранить", command=self._save_yadisk_settings
        ).pack(side="left", padx=6)
        self._btn_dl_private = ctk.CTkButton(
            row_priv,
            text="Загрузить (приватный путь)",
            command=self._download_db_from_yadisk_private,
        )
        ...
        ctk.CTkLabel(row_dl, text="Публичная ссылка на папку (опционально):").pack(
            side="left", padx=6
        )
        # Значение по умолчанию берем из пользовательских настроек, иначе дефолтное
        default_remote_file = (
            self._prefs.yandex_public_folder_url or "https://disk.yandex.ru/"
        )
        self._yd_remote_file_var = ctk.StringVar(value=default_remote_file)
        self._btn_paste_public = ctk.CTkButton(
            row_dl,
            text="Вставить из буфера",
            command=lambda: self._yd_remote_file_var.set(
                (self.clipboard_get() or "").strip()
            ),
        )
        self._btn_paste_public.pack(side="left", padx=6)
        self._yd_remote_file_entry = ctk.CTkEntry(
            row_dl, textvariable=self._yd_remote_file_var
        )
        self._yd_remote_file_entry.pack(side="left", fill="x", expand=True, padx=6)
        ctk.CTkButton(
            row_dl,
            text="Загрузить базу с Яндекс.Диска",
            command=self._download_db_from_yadisk,
        ).pack(side="left", padx=6)
        # Подсказка убрана
        ...
        ctk.CTkButton(
            btns_db, text="Выбрать...", command=self._choose_existing_db
        ).pack(side="left", padx=6)
        ctk.CTkButton(btns_db, text="Создать...", command=self._create_new_db).pack(
            side="left", padx=6
        )
        ctk.CTkButton(btns_db, text="Применить", command=self._apply_db_settings).pack(
            side="left", padx=6
        )
        ctk.CTkButton(
            btns_db, text="Проверить подключение", command=self._test_db_connection
        ).pack(side="left", padx=6)
        ...
        ctk.CTkLabel(self._backups_row, text="Выберите версию:").pack(
            side="left", padx=6
        )
        self._backup_choice = ctk.StringVar(value="")
        self._backup_map = {}
        self._opt_backups = ctk.CTkOptionMenu(
            self._backups_row,
            values=["(бэкапы не найдены)"],
            variable=self._backup_choice,
        )
        self._opt_backups.pack(side="left", padx=6)
        ctk.CTkButton(
            self._backups_row, text="Обновить список", command=self._refresh_backup_list
        ).pack(side="left", padx=6)
        ctk.CTkButton(
            self._backups_row,
            text="Перейти на эту версию данных",
            fg_color="#2563eb",
        ...
        ctk.CTkLabel(pw_box, text="Пароль пользователя (Полный доступ)").pack(
            anchor="w", pady=(0, 8)
        )
        ...
        ctk.CTkButton(
            rowp, text="Сменить пароль...", command=self._change_user_password
        ).pack(side="left", padx=6)
        ctk.CTkLabel(rowp, text="Для смены требуется текущий пароль").pack(
            side="left", padx=6
        )
        ...
        rowps.pack(fill="x")
        ctk.CTkButton(
            rowps, text="Установить пароль...", command=self._set_user_password
        ).pack(side="left", padx=6)
        ctk.CTkLabel(rowps, text="Если пароль ещё не установлен").pack(
            side="left", padx=6
        )
        ...
        ctk.CTkLabel(hint_row, text="Подсказка: Пароль админа М@2").pack(
            side="left", padx=6
        )
        ctk.CTkButton(
            hint_row,
            text="Войти в режим админа чтобы сбросить пароль",
            command=self._admin_reset_password_flow,
        ).pack(side="left", padx=6)
        ...
        self._opt_list_font = ctk.CTkOptionMenu(
            row,
            values=[str(i) for i in range(10, 21)],
            variable=self._list_font_var,
            command=lambda _: self._save_prefs(),
        )
        self._opt_list_font.pack(side="left")
        ctk.CTkLabel(row, text="Размер шрифта кнопок/надписей").pack(
            side="left", padx=(16, 6)
        )
        self._ui_font_var = ctk.StringVar(value=str(self._prefs.ui_font_size))
        self._opt_ui_font = ctk.CTkOptionMenu(
            row,
            values=[str(i) for i in range(10, 21)],
            variable=self._ui_font_var,
            command=lambda _: self._save_prefs(),
        )
                ...
                for child in list(self._backups_row.winfo_children()):
                    try:
                        if (
                            ...
                            and child.cget("text") == "Перейти на эту версию данных"
                        ):
                            _disable_widget(child)
    ...
    def _save_prefs(self) -> None:
        try:
            prefs = UserPrefs(
                list_font_size=int(self._list_font_var.get()),
                ui_font_size=int(self._ui_font_var.get()),
            )
            save_prefs(prefs)
            # Применить на лету
            try:
                ...
                self.status.configure(
                    text="Настройки интерфейса применены и сохранены."
                )
            except Exception:
                self.status.configure(
                    text="Настройки сохранены. Перезапуск может потребоваться для полного применения."
                )
        except Exception as exc:
    ...
    def _choose_existing_db(self) -> None:
        ...
        set_db_path(p)
        self.status.configure(
            text="Путь к существующей БД применён. Перезапустите приложение."
        )
    ...
    def _create_new_db(self) -> None:
        ...
        try:
            ...
            self._db_path_var.set(str(p))
            self.status.configure(
                text="Новая база создана и путь сохранён. Перезапустите приложение."
            )
        except Exception as exc:
        ...
        try:
            ...
            if new_path.exists():
                ...
                if not messagebox.askyesno(
                    ...
                    self.status.configure(
                        text="Создание отменено. Путь к БД не изменён."
                    )
                    return
                try:
            ...
            self.status.configure(
                text="Путь к БД применён. Перезапустите приложение, чтобы все окна использовали новую БД."
            )
        except Exception as exc:
    ...
    def _save_yadisk_settings(self) -> None:
        if self._readonly:
            messagebox.showwarning(
                "Яндекс.Диск", "Режим только для чтения — действие недоступно"
            )
            return
        prefs = load_prefs()
        ...
        prefs.yandex_remote_dir = (
            self._yd_dir_var.get() or "/SdelkaBackups"
        ).strip() or "/SdelkaBackups"
        ...
        try:
            ...
            self.status.configure(
                text=f"Ошибка сохранения настроек Яндекс.Диска: {exc}"
            )
    ...
    def _upload_latest_backup_to_yadisk(self) -> None:
        ...
        token = (
            prefs.yandex_oauth_token or CONFIG.yandex_default_oauth_token or ""
        ).strip()
        if not token:
            messagebox.showwarning(
                "Яндекс.Диск", "Укажите OAuth токен и нажмите 'Сохранить'."
            )
            return
        ...
        def run():
            try:
                ...
                client = YaDiskClient(
                    YaDiskConfig(oauth_token=token, remote_dir=remote_dir)
                )
                ...
                def _ok():
                    try:
                        messagebox.showinfo(
                            "Яндекс.Диск", f"Бэкап загружен: {remote_path}"
                        )
                        self.status.configure(
                            text=f"Выгрузка на Яндекс.Диск завершена: {remote_path}"
                        )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
                            "Ignored unexpected error: %s", exc
                ...
                def _err():
                    try:
                        ...
                        self.status.configure(
                            text=f"Ошибка выгрузки на Яндекс.Диск: {exc}"
                        )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
                            "Ignored unexpected error: %s", exc
            ...
            try:
                ...
                def _res():
                    try:
                        if ok:
                            messagebox.showinfo(
                                "Яндекс.Диск", f"Проверка пройдена: {msg}"
                            )
                        else:
                            messagebox.showerror(
                                "Яндекс.Диск", f"Проверка не пройдена: {msg}"
                            )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
                            "Ignored unexpected error: %s", exc
    ...
    def _download_db_from_yadisk(self) -> None:
        ...
        public_url = (
            self._yd_remote_file_var.get() or prefs.yandex_public_folder_url or ""
        ).strip()
        ...
        def run():
            ...
            try:
                ...
                if token:
                    ...
                    for cand in candidates:
                        ...
                        try:
                            ...
                            last_err = e_priv
                            log.warning(
                                "Yadisk private candidate failed: %s (%s)",
                                private_path,
                                e_priv,
                            )
                ...
                try:
                    eff_token = (
                        prefs.yandex_oauth_token
                        or CONFIG.yandex_default_oauth_token
                        or ""
                    ).strip()
                    if not public_url:
                        raise RuntimeError(
                            "Не удалось загрузить приватно. Укажите ссылку на Яндекс-папку (публичную) и повторите."
                        )
                    ...
                    def _err():
                        try:
                            messagebox.showerror(
                                "Загрузка базы",
                                f"Не удалось загрузить базу. Проверьте токен/настройки, либо укажите ссылку на Яндекс-папку и повторите.\n\nПодробности: {e_pub}",
                                parent=self,
                            )
                        except Exception as exc:
                            logging.getLogger(__name__).exception(
            ...
            try:
                ...
                if not (len(head) >= 16 and head[:16] == b"SQLite format 3\x00"):
                    ...
                    raise RuntimeError(
                        "Загруженный файл не является базой SQLite. Проверьте содержимое папки."
                    )
                ...
                def _err_nf():
                    try:
                        messagebox.showerror(
                            "Загрузка базы",
                            "Файл не был загружен. Повторите попытку.",
                            parent=self,
                        )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
            ...
            try:
                ...
                def _ok():
                    try:
                        messagebox.showinfo(
                            "Загрузка базы",
                            "База данных успешно загружена и заменена. Перезапустите приложение для применения во всех окнах.",
                            parent=self,
                        )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
    ...
    def _download_db_from_yadisk_private(self) -> None:
        ...
        prefs = load_prefs()
        private_path = (
            self._yd_private_var.get()
            or prefs.yandex_private_file_path
            or "/SdelkaBackups/sdelka_base.db"
        ).strip()
        # Нормализуем вид пути
        private_path = (
            private_path if private_path.startswith("/") else "/" + private_path
        )
        token = (
            prefs.yandex_oauth_token or CONFIG.yandex_default_oauth_token or ""
        ).strip()
            ...
            try:
                ...
                def _ok():
                    try:
                        messagebox.showinfo(
                            "Загрузка базы",
                            "База обновлена из приватного пути. Перезапустите приложение.",
                        )
                    except Exception as exc:
                        logging.getLogger(__name__).exception(
                            "Ignored unexpected error: %s", exc
    ...
    def _merge_db(self) -> None:
        ...
        messagebox.showinfo(
            "Слияние",
            f"Готово. Обновлено справочников: {refs}, добавлено нарядов: {orders}",
        )
    ...
    def _ask_open(
        ...
        if filter_name:
            pat = patterns or (
                f"*{default_ext}" if default_ext else "*.xlsx;*.xls;*.ods"
            )
    ...
    def _import_unified(self) -> None:
        if self._readonly:
            messagebox.showwarning(
                "Импорт", "Режим только для чтения — импорт недоступен"
            )
            return
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                (
                    "Поддерживаемые",
                    "*.txt;*.csv;*.xls;*.xlsx;*.ods;*.docx;*.odt;*.html;*.xml;*.pdf;*.dbf;*.json",
                ),
                ("Все файлы", "*.*"),
            ],
        )
        ...
        ctk.CTkCheckBox(
            dlg, text="Черновой прогон (без записи в БД)", variable=dry
        ).pack(anchor="w", padx=12)
        ctk.CTkLabel(dlg, text="Профиль").pack(anchor="w", padx=10, pady=(10, 2))
        ctk.CTkOptionMenu(
            dlg, values=["Авто", "Наряды", "Цена-лист", "Справочники"], variable=preset
        ).pack(anchor="w", padx=12)
        ...
        ctk.CTkButton(btns, text="Отмена", command=dlg.destroy).pack(
            side="right", padx=6
        )
        dlg.transient(self)
        dlg.grab_set()
        ...
        ctk.CTkLabel(win, text="Выполняется импорт...").pack(
                ...
                def _show_success():
                    try:
                        if report_path:
                            ...
                            messagebox.showinfo(
                                "Импорт (черновой)", f"Готово. Отчёт: {report_path}"
                            )
                        else:
    ...
    def _export_table(self, table: str) -> None:
        ...
        path = self._ask_save(
            f"Сохранить {table_rus}", ".xlsx", "Excel", initialfile=initial
        )
        if not path:
            return
        try:
    ...
    def _set_user_password(self) -> None:
        ...
        if user_password_is_set():
            messagebox.showinfo(
                "Пароль", "Пароль уже установлен. Используйте 'Сменить пароль...'."
            )
            return
        new1 = simpledialog.askstring(
            "Установка пароля", "Введите новый пароль:", parent=self, show="*"
        )
        if new1 is None or new1.strip() == "":
            return
        new2 = simpledialog.askstring(
            "Установка пароля", "Повторите новый пароль:", parent=self, show="*"
        )
        if new2 is None:
            return
        if new1 != new2:
    ...
    def _change_user_password(self) -> None:
        ...
        if not user_password_is_set():
            messagebox.showinfo(
                "Пароль",
                "Пароль ещё не установлен. Используйте 'Установить пароль...'.",
            )
            return
    ...
    def _admin_reset_password_flow(self) -> None:
        ...
        ctk.CTkLabel(dlg, text="Вы не можете вносить изменения в базу данных!").pack(
            pady=(16, 10)
        )
        ...
        ctk.CTkButton(
            dlg, text="Открыть программу в режиме просмотра.", command=_open_ro
        ).pack(pady=(6, 10))
        ...
        for d in search_dirs:
            ...
            try:
                ...
                for sub in d.rglob("*.db"):
                    try:
                        if sub.is_file() and re.fullmatch(
                            r"backup_base_sdelka_\d{4}_\d{4}", sub.stem
                        ):
    ...
    def _rebuild_backups_ui(self, options: list[str]) -> None:
        ...
        self._opt_backups = ctk.CTkOptionMenu(
            row, values=options, variable=self._backup_choice
        )
        self._opt_backups.pack(side="left", padx=6)
        ...
        for b in right_buttons:
            try:
                ...
                try:
                    if (
                        getattr(self, "_readonly", False)
                        and isinstance(b, ctk.CTkButton)
                        and b.cget("text") == "Перейти на эту версию данных"
                    ):
                        b.configure(
                            state="disabled",
                            fg_color=("#a3a3a3", "#3a3a3a"),
                            hover=False,
                        )
                except Exception as exc:
                    logging.getLogger(__name__).exception(
    ...
    def _restore_selected_backup(self) -> None:
        if self._readonly:
            messagebox.showwarning(
                "Восстановление БД", "Режим только для чтения — действие недоступно"
            )
            return
        ...
        if not backup_path or "не найдены" in label:
            messagebox.showwarning(
                "Восстановление БД", "Выберите доступную версию из списка"
            )
            return
        ...
        try:
            ...
            self.status.configure(
                text="База восстановлена из бэкапа. Перезапустите приложение для применения во всех окнах."
            )
        except Exception as exc:
    ...
    def _import_contracts_csv(self) -> None:
        ...
        try:
            ...
            with get_connection() as conn:
                ...
                self.status.configure(
                    text=f"Импорт CSV контрактов завершен. Импортировано: {imported}, обновлено: {updated}"
                )
        except Exception as e:
    ...
    def _export_contracts_csv(self) -> None:
        ...
        try:
            ...
            with get_connection() as conn:
                ...
                self.status.configure(
                    text=f"Экспорт CSV контрактов завершен: {result_path}"
                )
        except Exception as e:
    ...
    def _import_products_contracts(self) -> None:
        ...
        try:
            from import_export.products_contracts_import import (
                import_products_from_contracts_csv,
            )
            ...
            pb.set(0)
            note_var = tk.StringVar(value="")
            ctk.CTkLabel(win, textvariable=note_var).pack(
                ...
                try:
                    ...
                    def _success():
                        try:
                            ...
                            self.status.configure(
                                text=f"Импорт изделий с контрактами завершен. Изделий: {result['products']}, контрактов: {result['contracts']}"
                            )
                        except Exception as exc:
                            logging.getLogger(__name__).exception(
                                "Ignored unexpected error: %s", exc
                    ...
                    def _err():
                        try:
                            messagebox.showerror("Импорт", str(e))
                            self.status.configure(
                                text=f"Ошибка импорта изделий с контрактами: {e}"
                            )
                        except Exception as exc:
                            logging.getLogger(__name__).exception(
                                "Ignored unexpected error: %s", exc
