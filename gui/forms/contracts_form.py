...
class ContractsForm(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ...
        # Первая строка
        ctk.CTkLabel(form, text="Шифр контракта").grid(
        ...
        # Вторая строка
        ctk.CTkLabel(form, text="Вид контракта").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        self.contract_type_entry = ctk.CTkEntry(
            form, textvariable=self.contract_type_var, width=200
        )
        self.contract_type_entry.grid(row=1, column=1, sticky="w", padx=5, pady=5)

        ctk.CTkLabel(form, text="Исполнитель").grid(
            row=1, column=2, sticky="w", padx=5, pady=5
        )
        self.executor_entry = ctk.CTkEntry(
            form, textvariable=self.executor_var, width=300
        )
        ...
        ctk.CTkLabel(form, text="Номер контракта").grid(
            row=2, column=2, sticky="w", padx=5, pady=5
        )
        self.contract_number_entry = ctk.CTkEntry(
            form, textvariable=self.contract_number_var, width=300
        )
        self.contract_number_entry.grid(row=2, column=3, sticky="w", padx=5, pady=5)

        # Четвертая строка
        ctk.CTkLabel(form, text="Отдельный счет").grid(
            row=3, column=0, sticky="w", padx=5, pady=5
        )
        self.bank_account_entry = ctk.CTkEntry(
            form, textvariable=self.bank_account_var, width=200
        )
        self.bank_account_entry.grid(row=3, column=1, sticky="w", padx=5, pady=5)

        ctk.CTkLabel(form, text="Дата начала").grid(
        ...
        self.start_entry.bind(
            "<FocusIn>",
            lambda e: self._open_date_picker(self.start_var, self.start_entry),
        )

        # Пятая строка
        ctk.CTkLabel(form, text="Дата окончания").grid(
        ...
        self.end_entry.bind(
            "<FocusIn>", lambda e: self._open_date_picker(self.end_var, self.end_entry)
        )

        ctk.CTkLabel(form, text="Описание").grid(
        ...
        if self._readonly:
            for w in (
                self.code_entry,
                self.name_entry,
                self.contract_type_entry,
                self.executor_entry,
                self.igk_entry,
                self.contract_number_entry,
                self.bank_account_entry,
                self.start_entry,
                self.end_entry,
                self.desc_entry,
            ):
    ...
    def _on_code_key(self, _evt=None) -> None:
        ...
        for val in vals:
            create_suggestion_button(
                self.suggest_code_frame,
                text=val,
                command=lambda s=val: self._pick_code(s),
        ...
        for label in get_recent("contracts.code", prefix, CONFIG.autocomplete_limit):
            if label not in vals:
                create_suggestion_button(
                    self.suggest_code_frame,
                    text=label,
                    command=lambda s=label: self._pick_code(s),
                ).pack(fill="x", padx=2, pady=1)
                shown += 1
        ...
        if shown == 0:
            ...
            for row in all_contracts:
                ...
                create_suggestion_button(
                    self.suggest_code_frame,
                    text=row["code"],
                    command=lambda s=row["code"]: self._pick_code(s),
                ).pack(fill="x", padx=2, pady=1)
                shown += 1
                ...
                if self._selected_id:
                    ...
                    if q.get_contract_by_code(conn, code):
                        messagebox.showwarning(
                            "Дубликат",
                            "Контракт уже существует. Выберите его для редактирования.",
                        )
                        return
                    ref.create_contract(
                        conn,
