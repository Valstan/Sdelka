...
class ProductsForm(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(form, text="Контракт").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.contract_entry = ctk.CTkEntry(form, textvariable=self.contract_var, width=360)
        ...
        self.tree = ttk.Treeview(table_frame, columns=("name", "product_no", "contract_code"), show="headings")
    ...
    def _on_global_click(self, event=None) -> None:
        ...
        if widget is None:
            ...
            self.suggest_no_frame.place_forget()
            return
        for frame in (self.suggest_name_frame, self.suggest_no_frame, self.suggest_contract_frame):
            w = widget
            while w is not None:
                if w == frame:
                    return
        ...
        for r in rows:
            try:
                ...
                try:
                    ...
                    cc = ""
            self.tree.insert("", "end", iid=str(r["id"]), values=(r["name"], r["product_no"], cc or ""))
    ...
    def _on_select(self, _evt=None) -> None:
        ...
        try:
            with get_connection() as conn:
                if contract_code:
                    row = conn.execute("SELECT id FROM contracts WHERE code_norm = ?", (contract_code.casefold(),)).fetchone()
                    if row:
                        self.selected_contract_id = int(row["id"] if isinstance(row, dict) else row[0])
        except Exception:
            self.selected_contract_id = None
    ...
    def _save(self) -> None:
        ...
        if contract_label:
            try:
                with get_connection() as conn:
                    row = conn.execute("SELECT id FROM contracts WHERE code_norm = ?", (contract_label.casefold(),)).fetchone()
                    if row:
                        contract_id = int(row["id"] if isinstance(row, dict) else row[0])
                    else:
                        q.upsert_contract(conn, contract_label, None, None, None)
                        row2 = conn.execute("SELECT id FROM contracts WHERE code_norm = ?", (contract_label.casefold(),)).fetchone()
                        if row2:
                            contract_id = int(row2["id"] if isinstance(row2, dict) else row2[0])
            except Exception:
                contract_id = None
        try:
            with get_connection() as conn:
    ...
    def _on_contract_key(self, _evt=None) -> None:
        ...
        with get_connection() as conn:
            rows = suggestions.suggest_contracts(conn, prefix, CONFIG.autocomplete_limit)
        shown = 0
        ...
        if shown == 0:
            with get_connection() as conn:
                all_rows = suggestions.suggest_contracts(conn, "", CONFIG.autocomplete_limit)
